...
final class CatClient: WebServiceClient {
    ...
    func checkIfFollowingUser(userLogin: String) async -> NetworkError? {
        await networkManager.dataRequest(request: CatRouter.checkIfFollowingUser(login: userLogin))
    }
    
    func checkIfStarredRepository(fullName: String) async -> NetworkError? {
        await networkManager.dataRequest(request: CatRouter.checkIfStarredRepository(fullName: fullName))
    }
    
    func followUser(userLogin: String) async -> NetworkError? {
        await networkManager.dataRequest(request: CatRouter.followUser(login: userLogin))
    }
    
    func unFollowUser(userLogin: String) async -> NetworkError? {
        await networkManager.dataRequest(request: CatRouter.unFollowUser(login: userLogin))
    }
    
    func starRepository(fullName: String) async -> NetworkError? {
        await networkManager.dataRequest(request: CatRouter.starRepository(fullName: fullName))
    }
    
    func unStarRepository(fullName: String) async -> NetworkError? {
        await networkManager.dataRequest(request: CatRouter.unStarRepository(fullName: fullName))
    }
    ...
    func downloadRepositoryLicense(fullName: String, branch: String) async -> DataResult {
        await networkManager.dataRequest(request: CatRouter.downloadRepositoryLicense(fullName: fullName, branch: branch))
    }
    
    func downloadRepositoryREADME(fullName: String, branch: String) async -> DataResult {
        await networkManager.dataRequest(request: CatRouter.downloadRepositoryREADME(fullName: fullName, branch: branch))
    }
